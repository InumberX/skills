...
| `package.json` の依存に `react-router` がある | `rules/security/react-router.md` |

**設定ファイル（`next.config.*` など）の有無で判定しない。** これらはいずれも省略可能で、無いまま運用しているリポジトリではルールが読まれなくなる。依存パッケージなら必ず存在する。
...
- **`rules/security/` にファイルを足したら**「観点の選択」の検出条件テーブルにも行を足す。検出条件が書かれていないファイルは読まれない。検出条件は**依存パッケージ**で書く（設定ファイルは省略可能なことが多く、取りこぼす）
