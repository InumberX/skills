...
例外はセキュリティ観点で、これだけは `rules/security/` として本スキルに同梱している。参照するのがフレームワーク・プラットフォームの規定する識別子とファイル名（`ssr`・`define`・`run_worker_first`・`next.config.*`・`middleware.ts` など、どのリポジトリでも同じ名前になるもの）だけで、**そのリポジトリ固有のパス・識別子を引用しない**ため中央で保守できる。
...
- 例外は `rules/security/` で、ここだけは本リポジトリに観点本文を置く。線引きは「何を引用するか」で判断する。**フレームワーク・プラットフォームが規定する識別子とファイル名までなら中央**（`ssr`・`run_worker_first` のような設定キー、`next.config.*`・`middleware.ts`・`route.ts` のような公式に名前が決まっているファイル。どのリポジトリでも同じ名前になり、そのリポジトリの都合では変わらない）。**そのリポジトリにしか存在しないパス・識別子（コンポーネント名、独自に切ったサーバーモジュールのパスなど）を引用するならプロジェクト側。** プロジェクト固有の不変条件（このリポジトリのこのファイルがこの前提を守っている、という類）は中央に書かない
