...
| `wrangler.jsonc` / `wrangler.json` / `wrangler.toml` がある | `rules/security/cloudflare-workers.md` |
...
- セキュリティ指摘は「悪用され得る筋道」まで書く。書けないものは指摘ではなく Should / Nit として出す
...
- **`rules/security/` にファイルを足したら**「観点の選択」の検出条件テーブルにも行を足す。検出条件が書かれていないファイルは読まれない
