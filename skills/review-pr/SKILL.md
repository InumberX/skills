...
例外はセキュリティ観点で、これだけは `rules/security/` として本スキルに同梱している。参照するのがフレームワーク・プラットフォームの識別子（`ssr`・`define`・`run_worker_first` など）だけで、実コードのパスを引用しないため中央で保守できる。
...
- 例外は `rules/security/` で、ここだけは本リポジトリに観点本文を置く。線引きは「何を引用するか」で判断する。**フレームワーク・プラットフォームの識別子しか引用しないなら中央、実コードのパス・識別子を引用するならプロジェクト側。** プロジェクト固有の不変条件（このリポジトリのこのファイルがこの前提を守っている、という類）は中央に書かない
