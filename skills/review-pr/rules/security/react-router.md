...
原則と判定方法は `boundary.md`「遷移先を利用者入力から組み立てない」にある。ここでは React Router での書き方を示す。

`loader` は**全リクエストで動く root のものを含む**ため、ルート単位のマッチングが失敗する経路でも先に評価される。「このパスはどのルートにも当たらないから安全」は成立しない。
...
オリジンで比較すれば `//evil.com`・`/\evil.com`・`////evil.com` がまとめて弾ける（`javascript:` スキームもオリジンが一致しないため落ちる）。

**ロケール接頭辞の除去など、パスを加工してから渡す処理が特に危ない。** 除去の置換がスラッシュを1つ増やし、後段の `//` 折り畳みが追いつかずにプロトコル相対 URL が残る、という形になる。加工が1段でも入っていたら、加工後の文字列を実際に解決してオリジンを確かめる。
